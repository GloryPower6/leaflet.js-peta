...
selectIcon.addEventListener('change', onChangeSelectedIcon)
...
function populateMarker() {
  ...
  data.forEach((data) => {
    ...
    .on("click", (e) => {
      ...
      removeLayersFromGroups(markersGroup);
      ...
      leafletControlsReset();

      if (selectedMarker !== undefined) {
        const latlngSelectedMarker = selectedMarker.getLatLng();

        function searchMarkerByLatLng(layerGroup) {
          layerGroup.eachLayer((e) => {
            ...
            if (
              ...
              e.removeFrom(layerGroup);
            }
          });
        }

        const groupLayers = [
          markersGroup2, layerGroupPenginapan, layerGroupSekolah, layerGroupTempatIbadah
        ];

        groupLayers.forEach(value => {
          searchMarkerByLatLng(value);
        })
      }
    ...
    setCategoryIcon(marker, icon);
    setCategoryIcon(marker2, icon);

    addToLayersGroup(marker2, icon);
...
function onChangeSelectedIcon() {
  const icon = selectIcon.value;
  if (selectedMarker) {
    setCategoryIcon(selectedMarker, icon);
  }
}

function leafletControlsReset() {
  reAddLayerGroupControl();
}
